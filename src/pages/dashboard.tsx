...
import { useRoqPlatformUserProfiles, useFooCount } from 'lib/roq';

import { FooListPage as ShowCaseList } from 'pages/foos';

function DashboardPage() {
  const { session } = useSession();

  const fooStat = useFooCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Foos',
      result: fooStat?.data,
      isLoading: fooStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`foos`} passHref legacyBehavior>
                  <Button
